...
class EdgeFinder:
    ...
    def calculate_edge_metric_for_dataloader(self, model, layer, mask, to_normalise=True):
        if self.aggregation_mode == 'mean':
            accumulated = None
            for data, target in self.dataloader:
                data, target = data.to(self.device), target.to(self.device)
                metric = self.metric.calculate(model, layer, mask, data, target) 
    ...
    def choose_edges_threshold(self, model, layer, threshold, layer_mask):
        assert 0 < threshold <= 1
        avg_metric = self.calculate_edge_metric_for_dataloader(model, layer, layer_mask)
